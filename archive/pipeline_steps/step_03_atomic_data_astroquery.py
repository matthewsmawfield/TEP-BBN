"""
Step 03: Atomic data download for TEP-BBN (using astroquery)

Downloads and registers atomic data (wavelengths, oscillator strengths, etc.) from
NIST using astroquery.nist for programmatic access.

CRITICAL: This step uses astroquery.nist to query NIST database programmatically.
"""
...
def download_atomic_data():
    ...
    CRITICAL: This step requires astroquery to be installed.
    """
    print("Step 03: Atomic data download (using astroquery.nist)")
    print("=" * 60)
    print("CRITICAL: This step uses astroquery.nist for programmatic access.")
    print("=" * 60)
    print()
    ...
    try:
        ...
        print("See DATA_ACQUISITION_REQUIREMENTS.md for manual download steps.")
        return None
    ...
    required_elements = {
        'H_I': {
            ...
            'wavelengths': [1215.67, 1025.72, 972.537, 949.743, 937.803],  # Angstroms (vacuum)
        ...
        'D_I': {
            ...
            'ion': 'I',
            'transitions': ['Lyα', 'Lyβ', 'Lyγ', 'Lyδ', 'Lyε'],
            'wavelengths': [1215.34, 1025.44, 972.268, 949.479, 937.542],  # Angstroms (isotope-shifted)
            'wavelength_ranges': [(1215.0, 1216.0), (1025.0, 1026.0), (972.0, 973.0), (949.0, 950.0), (937.0, 938.0)]
        },
        'O_I': {
            'element': 'Oxygen',
            ...
            'wavelengths': [1302.1685, 1304.8576],
    ...
    for element_key, element_data in required_elements.items():
                ...
                if len(table) > 0:
                    ...
                    for row in table:
                        line_data = {
                            ...
                            'wavelength_air': float(row['Observed Air']) if 'Observed Air' in row.colnames else None,
        ...
        with open(data_file, 'w') as f:
            ...
            f.write(f"# Source: NIST ASD {element_data['version']}\n")
            ...
            for line in all_lines:
                if line_data['wavelength'] is not None:
                    f.write(f"{line_data['wavelength']:.4f}  ")
                    if line_data['oscillator_strength'] is not None:
                        f.write(f"{line_data['oscillator_strength']:.4e}  ")
                    else:
                        f.write("N/A  ")
                    f.write(f"{line_data['transition']}  ")
                    if line_data['lower_level'] is not None:
                        f.write(f"{line_data['lower_level']}  ")
                    if line_data['upper_level'] is not None:
                        f.write(f"{line_data['upper_level']}")
                    f.write("\n")
        ...
        if 'N/A' in content or len(all_lines) == 0:
            print(f"  ⚠ WARNING: Data may be incomplete or contain N/A values")
            atomic_data_registry['data'][element_key] = {
                'status': 'incomplete',
                'file': str(data_file),
                'n_lines': len(all_lines),
                'warning': 'Data may be incomplete or contain N/A values'
            }
        else:
            print(f"  ✓ Real atomic data obtained: {len(all_lines)} lines")
            atomic_data_registry['data'][element_key] = {
                'status': 'real_data',
                'file': str(data_file),
                'n_lines': len(all_lines),
