...
class TemporalFitter:
    ...
    def pack_params(self, p_dict):
        ...
        for i in range(3):
            ...
            arr.append(p_dict['sigma_A'][i]) # Temporal variance parameter
    ...
    def get_components(self, p_dict):
        ...
        for i in range(3):
            comps.append({
                ...
                'b': p_dict['sigma_A'][i], # Total b is pure temporal + turbulence
                'v': p_dict['HI_v'][i]
            })
        return comps
    ...
    def get_d_components(self, p_dict):
        ...
        return [{
            'N': 10**p_dict['DI_logN'],
            # CRITICAL TEP DIFFERENCE: No classical thermal scaling (1/sqrt(2))
            # Temporal variance and macroscopic turbulence are mass-independent.
            'b': p_dict['sigma_A'][0], 
...
def run_fit():
    ...
    fitter = TemporalFitter(manifest_path)
    
    # Initial Guess
    p0_dict = {
        'HI_logN': [17.36, 14.0, 14.0],
        'sigma_A': [15.0, 10.0, 10.0],
    ...
    print(f"Starting TEMPORAL fit with {len(p0)} parameters over {len(fitter.chunks)} chunks...")
    print("Note: Deuterium line width is strictly equal to Hydrogen line width (sigma_A).")
    ...
    for i, label in enumerate(['A', 'B', 'C']):
        print(f"HI {label}: logN = {p_final['HI_logN'][i]:.3f}, sigma_A = {p_final['sigma_A'][i]:.2f}, v = {p_final['HI_v'][i]:.2f}")
    print(f"DI A: logN = {p_final['DI_logN']:.3f} (D/H = {p_final['DI_logN'] - p_final['HI_logN'][0]:.3f} dex)")
    print(f"DI A line width (sigma_A): {p_final['sigma_A'][0]:.2f} km/s")
