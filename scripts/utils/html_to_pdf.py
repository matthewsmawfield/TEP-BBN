...
Usage:
    python html_to_pdf.py input.html output.pdf --scale 1.5 --dpi 300
"""
        ...
        try:
            ...
            if options.get("wait_for_selector"):
                ...
                await page.wait_for_selector(
                    options["wait_for_selector"], timeout=60000
                )
                # Additional wait to ensure all content is loaded
                await page.wait_for_timeout(2000)
            elif options.get("wait_time"):
                logger.info(
                    f"Waiting {options['wait_time']} seconds for content to load"
                )
            ...
            try:
                ...
                logger.warning(
                    "Could not detect loading indicator state, proceeding anyway"
                )
            ...
            logger.info(
                f"Generating PDF with options: {json.dumps(pdf_options, indent=2)}"
            )
...
Examples:
  ...
  python html_to_pdf.py input.html output.pdf --preset high_quality --scale 1.5
  
  # Custom dimensions and margins
  python html_to_pdf.py input.html output.pdf --format A3 --margin-top 2cm --scale 1.2
  ...
  python html_to_pdf.py input.html output.pdf --header-footer --wait-time 5
    ...
    parser.add_argument(
        "--preset",
        choices=create_preset_configs().keys(),
        help="Use preset configuration",
    )

    # Page format and dimensions
    parser.add_argument(
        "--format", default="A4", help="Page format (A4, A3, Letter, Legal, Tabloid)"
    )
    ...
    # Headers and footers
    parser.add_argument(
        "--header-footer", action="store_true", help="Enable headers and footers"
    )
    ...
    # Wait options
    parser.add_argument(
        "--wait-time", type=float, help="Wait time in seconds before PDF generation"
    )
    parser.add_argument(
        "--wait-for-selector", help="CSS selector to wait for before PDF generation"
    )

    # Advanced options
    parser.add_argument(
        "--page-ranges", help='Page ranges to print (e.g., "1-3, 5, 8-")'
    )
    parser.add_argument(
        "--css-page-size", action="store_true", help="Prefer CSS page size over format"
    )
    parser.add_argument(
        "--custom-css-file", help="Path to custom CSS file for PDF generation"
    )

    # Viewport
    parser.add_argument(
        "--viewport-width", type=int, default=1920, help="Browser viewport width"
    )
    parser.add_argument(
        "--viewport-height", type=int, default=1080, help="Browser viewport height"
    )
