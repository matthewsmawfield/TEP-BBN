#!/usr/bin/env python3
"""
TEP-GNSS-RINEX Logging Utilities
================================
...
class TEPFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        level_name, color = level_mapping.get(
            record.levelno, ("INFO", self.COLORS["INFO"])
        )
...
def print_table(headers: list, rows: list, title: str = None):
    ...
    if _current_step_logger:
        ...
        header_str = (
            "|"
            + "|".join([f" {str(h):<{w-2}} " for h, w in zip(headers, col_widths)])
            + "|"
        )
        ...
        for row in rows:
            row_str = (
                "|"
                + "|".join(
                    [f" {str(cell):<{w-2}} " for cell, w in zip(row, col_widths)]
                )
                + "|"
            )
...
def check_memory_usage(context: str = "Unknown") -> None:
    ...
    if _current_step_logger is not None:
        _current_step_logger.debug(
            f"Memory usage in {context}: RSS={rss_mb:.2f} MB, VMS={vms_mb:.2f} MB"
        )
    else:
        print(
            f"[DEBUG] Memory usage in {context}: RSS={rss_mb:.2f} MB, VMS={vms_mb:.2f} MB"
        )
    gc.collect()
